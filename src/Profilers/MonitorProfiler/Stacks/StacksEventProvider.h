...
class StacksEventProvider
{
    ...
    private:
        ...
        const WCHAR* FunctionPayloads[9] = { _T("FunctionId"), _T("MethodToken"), _T("ClassId"), _T("ClassToken"), _T("ModuleId"), _T("Name"), _T("TypeArgs"), _T("ParameterTypes"), _T("StackTraceHidden") };
        std::unique_ptr<ProfilerEvent<UINT64, UINT32, UINT64, UINT32, UINT64, tstring, std::vector<UINT64>, std::vector<UINT64>, bool>> _functionEvent;

        //We cannot retrieve detailed information for some ClassIds. Flags is used to indicate these conditions.
        const WCHAR* ClassPayloads[6] = { _T("ClassId"), _T("ModuleId"), _T("Token"), _T("Flags"), _T("TypeArgs"), _T("StackTraceHidden") };
        std::unique_ptr<ProfilerEvent<UINT64, UINT64, UINT32, UINT32, std::vector<UINT64>, bool>> _classEvent;

        const WCHAR* TokenPayloads[6] = { _T("ModuleId"), _T("Token"), _T("OuterToken"), _T("Name"), _T("Namespace"), _T("StackTraceHidden") };
        std::unique_ptr<ProfilerEvent<UINT64, UINT32, UINT32, tstring, tstring, bool>> _tokenEvent;
